...
class Discomfort:
    def __init__(self, now=datetime.datetime.utcnow().replace(tzinfo=pytz.timezone("UTC")).astimezone(
    ...
    def disc(self, t_in, occ, node_time):
        ...
        if weekno < 5:
            now_time = (datetime.datetime.utcnow().replace(tzinfo=pytz.timezone("UTC")).astimezone(
                tz=pytz.timezone("America/Los_Angeles")) + timedelta(minutes=node_time)).time()
