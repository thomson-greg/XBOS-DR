...
class ThermalModel:
    ...
    def next_temperature(self, Tin, action, time, zone=0):
        ...
        if action == 'Heating' or action == '1':
            return round(func(
                [Tin, 1, 0, self.weather_predictions[(self.current_time + timedelta(self.interval_length * time)).hour],
                 self.interval_length],
                *self.popt) * self.thermal_precision) / self.thermal_precision
        elif action == 'Cooling' or action == '2':
            return round(func(
                [Tin, 0, 1, self.weather_predictions[(self.current_time + timedelta(self.interval_length * time)).hour],
                 self.interval_length],
                *self.popt) * self.thermal_precision) / self.thermal_precision
        else:
            return round(func(
                [Tin, 0, 0, self.weather_predictions[(self.current_time + timedelta(self.interval_length * time)).hour],
                 self.interval_length],
                *self.popt) * self.thermal_precision) / self.thermal_precision
