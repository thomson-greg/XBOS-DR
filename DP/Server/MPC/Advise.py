		...
		for action in action_set:
			...
			if this_path_cost <= self.g.node[from_node]['usage_cost']:
				...
				if this_path_cost == self.g.node[from_node]['usage_cost'] and self.g.node[from_node][
					...
					continue
				"""
				self.g.add_node(from_node, best_action=action, usage_cost=this_path_cost, best_successor=new_node)
...
class Advise:
	...
	def advise(self):
		...
		return self.advise_unit.g.node[self.root]["best_action"] #self.advise_unit.g[self.path[0]][self.path[1]]['action']
