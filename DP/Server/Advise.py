...
class EVA:
    ...
    def shortest_path(self, from_node):
        ...
        for action in action_set:
            ...
            demand_balancer = (((self.target - self.get_real_time(
                from_node.time / self.interval)).total_seconds() / 60) / self.billing_period)
            ...
            for i in range(self.noZones):
                discomfort[i] = self.disc.disc(new_temperature[i], self.occ.occ(from_node.time / self.interval),
                                               from_node.time)
            ...
            interval_overall_cost = ((1 - self.l) * (interval_usage_cost + this_path_demand_cost)) + (
            self.l * (sum(discomfort)))
...
class Advise:
    def __init__(self, cfg):
        ...
        Maximum_Safety_Temp, Minimum_Safety_Temp = cfg['advise']['maximum_Safety_Temp'], cfg['advise'][
            'minimum_Safety_Temp']
        ...
        self.current_time = datetime.datetime.utcnow().replace(tzinfo=pytz.timezone("UTC")).astimezone(
